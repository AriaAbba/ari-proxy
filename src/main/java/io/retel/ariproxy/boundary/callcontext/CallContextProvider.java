...
import io.retel.ariproxy.persistence.SetDone;
            ...
            .flatMap(
                option ->
                    option
                        .map(
                            (String callContext) ->
                                Future.successful(new CallContextProvided(callContext)))
                        .getOrElse(
                            () ->
                                ProviderPolicy.CREATE_IF_MISSING.equals(cmd.policy())
                                    ? update(cmd.resourceId(), UUID.randomUUID().toString())
                                        .map(
                                            (SetDone setDone) ->
                                                new CallContextProvided(setDone.getValue()))
                                    : Future.failed(
                                        new CallContextLookupError(
                                            "Failed to lookup call context..."))))
            .await();
